...
impl LocalExchangeInner {
    fn calc_market_price_slippage(&self, side: Side, market_price: Decimal) -> Decimal {
        let price: Decimal = match side {
            Side::Buy => market_price * (1 + self.slippage),
    ...
    fn adjust_freeze_margin_by_avg_price(
        ...
        if order.reduce_only || order.freeze_margin == 0 {
            return Ok(());
        }
...
impl Exchange for LocalExchange {
    ...
    async fn place_order(&self, order: Order) -> anyhow::Result<String> {
        ...
        if order.is_trigger() {
            ...
            if order.price > 0 && !is_tick_aligned(order.price, metadata.tick_size) {
                bail!(
    ...
    async fn append_position_margin(&self, symbol: &str, margin: Decimal) -> anyhow::Result<()> {
        ...
        let (liquidation_order_id, liquidation_price, cash_delta) = match inner.position.as_mut() {
            Some(position) => {
                ...
                if margin < 0 && margin.abs() > position.margin {
                    bail!(
    ...
    async fn set_leverage(&self, symbol: &str, leverage: u32) -> anyhow::Result<()> {
        ...
        if append_margin > 0 && inner.cash < append_margin {
            bail!(
...
mod tests {
    ...
    fn test_exchange_with(metadata: Metadata, kline_list: Vec<KLine>) -> ExchangeWrapper {
    ...
    fn test_exchange() -> ExchangeWrapper {
    ...
    async fn limit_order_not_fill_when_slightly_above_high() {
        ...
        exchange.next(SYMBOL, Level::Minute1).await.unwrap();
        let id = exchange.buy_limit(SYMBOL, 104.0 - 0.1, 1.0).await.unwrap();
    ...
    async fn sell_limit_not_fill_when_slightly_above_high_boundary() {
        ...
        exchange.next(SYMBOL, Level::Minute1).await.unwrap();
        let id = exchange.sell_limit(SYMBOL, 106.0 + 0.1, 1.0).await.unwrap();
    ...
    async fn strict_complex_flow_asserts_all_states_on_every_next() {
        ...
        let exchange = ExchangeWrapper::new(Arc::new(
            LocalExchange::new(data_source).cash(20000.0).leverage(10),
        ));
    ...
    async fn insane_state_machine_asserts_everything_each_next() {
        ...
        let exchange = ExchangeWrapper::new(Arc::new(
            LocalExchange::new(data_source).cash(30000.0).leverage(10),
        ));
    ...
    async fn range_boundary_yields_expected_klines_then_none() {
        let data_source = DataSource::new(default_metadata(), gen_kline_range());
        let exchange = ExchangeWrapper::new(Arc::new(
    ...
    async fn trigger_order_exact_low_boundary_triggers_but_below_does_not() {
        ...
        let below_id = exchange
            .buy_trigger_market(SYMBOL, 104.0 - 0.1, 1.0)
            .await
            .unwrap();
