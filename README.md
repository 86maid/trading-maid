...
A trend-following strategy based on the classic **Vegas Tunnel** (EMA 12/144/169). It only trades when the market is trending (the EMA band is not consolidating), enters on trend alignment with **RSI** and **MACD histogram** confirmation, and uses a **Martingale** position ladder (0.01 → 0.02 → 0.04 → 0.08) that doubles the size after each losing round. Take-profit is set at 2× the ATR-based stop distance. Uses `EMACache`, `rsi()`, `atr()`, `macd()` from the built-in indicators.
